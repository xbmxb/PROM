...
class BartEncoder(nn.Module):
    ...
    def forward(
        ...
        return BaseModelOutput(last_hidden_state=x, hidden_states=encoder_states, attentions=all_attentions, encoder_input_ids=input_ids)
...
class BartModel(PretrainedBartModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        decoder_outputs, x_0 = self.decoder(
            decoder_input_ids,
            encoder_outputs[0],
            attention_mask,
            decoder_padding_mask,
        ...
        if not return_dict:
            return decoder_outputs + encoder_outputs, x_0
...
class BartModel_pointer(PretrainedBartModel):
    def __init__(self, config: BartConfig):
        super().__init__(config)
        ...
        self.encoder = BartEncoder(config, self.shared)
        self.decoder = BartDecoder(config, self.shared)
        self.copy_indicator = nn.Sequential(
    ...
    def forward(
        ...
        **kwargs,
        ...
        p_indi = self.copy_indicator(encoder_outputs.last_hidden_state)

        if training_obj == "selector":
            return p_indi
        else:
            # decoder outputs consists of (dec_features, layer_state, dec_hidden, dec_attn)
            decoder_outputs, x_0 = self.decoder(
            ...
            if not return_dict:
                return decoder_outputs + encoder_outputs, x_0, p_indi
...
class BartForConditionalGeneration_Pointer(PretrainedBartModel):
    base_model_prefix = "model"
    authorized_missing_keys = [r"final_logits_bias", r"encoder\.version", r"decoder\.version"]

    def __init__(self, config: BartConfig, src_vocab_size=None):
        super().__init__(config)
        print('Using bart with pointer...')
        base_model = BartModel_pointer(config)
    ...
    def forward(
        ...
        **unused,
        ...
        if training_obj == "selector":
            ...
            if copylabels is not None:
                loss_fct = CrossEntropyLoss()
                # cp_loss=0
                cp_loss = loss_fct(p_indi.view(-1, p_indi.shape[-1]), copylabels.long().view(-1))
            return cp_loss, p_indi

        else:
            outputs, x_0_de, p_indi = self.model(
                input_ids,
                attention_mask=attention_mask,
                decoder_input_ids=decoder_input_ids,
                encoder_outputs=encoder_outputs,
            ...
            if copylabels is not None:
                ...
                cp_loss = loss_fct(p_indi.view(-1, p_indi.shape[-1]), copylabels.long().view(-1))
                # print(cp_loss.shape)
            p_gen_from_indi = p_indi[:,:,1] # bs, src
            #p_vocab
            ...
            hidden_states = outputs.encoder_last_hidden_state #.transpose(0, 1) #2, 1024, 768 -> 1024, 2, 768
            # compute context vectors. (N, T, S) x (N, S, E) -> (N, T, E)  #2, 12, 96, 1024 -> 2, 96, 768
            attn = outputs.cross_attentions[-1].mean(dim=1) #2, 12, 96, 1024 -> 2, 96, 1024
            context_vectors = torch.matmul(attn, hidden_states) #.transpose(0, 1) #2, 96, 1024 x 2, 1024, 768 -> 2, 96, 768 -> 96, 2, 768
            x_0_de_ = x_0_de.to(torch.float32)
            total_states = torch.cat((context_vectors, outputs.last_hidden_state, x_0_de_), dim=-1) # 2, 96, 768*3
            ...
            #modify attn
            p_gen_from_indi = p_gen_from_indi.unsqueeze(1).expand_as(attn) # bs tgt src
            attn_ = torch.cat([attn.unsqueeze(-1), p_gen_from_indi.unsqueeze(-1)], dim=-1)
            attn_ = self.copy_dist(attn_).squeeze(-1)

            # Get representation of src tokens as one hot encoding 2, 1024, 50265
            if input_ids.size(0) != attn_.size(0): #->beam search
                input_ids = encoder_outputs.encoder_input_ids
            one_hot = torch.zeros(input_ids.size(0), input_ids.size(1), self.config.vocab_size, device=input_ids.device)
            one_hot = one_hot.scatter_(dim=-1, index=input_ids.unsqueeze(-1), value=1)
            # p_copy from source is sum over all attention weights for each token in source 2, 12, 96, 1024
            # print(attn.size())
            # print(one_hot.size())
            attention_logits = torch.matmul(attn_, one_hot) # N T vocab
            # ######scatter_add_
            # attention_logits = torch.zeros_like(lm_logits, device=input_ids.device)
            # attn_index = input_ids.unsqueeze(1).expand_as(attn_)
            # attention_logits = attention_logits.scatter_add(2, attn_index, attn_)
            ######
            # print(attention_logits.size())
            # print(lm_logits.size())
            # # convert representation of token from original vocab to the whole vocab
            # lm_logits_whole_vocab = torch.matmul(lm_logits, self.orig_to_tgt_vocab_conversion_matrix).transpose(0,1)
            # # convert representation of token from src vocab to the whole vocab
            # p_copy_whole_vocab = torch.matmul(p_copy_src_vocab, self.src_to_whole_vocab_conversion_matrix).transpose(0,1)
            # os.exit(0)
            # Compute final probability
            logits = torch.add(lm_logits * p_gen, attention_logits * p_copy)
    ...
    def prepare_inputs_for_generation(
        self, encoder_input_ids, decoder_input_ids, past=None, copylabels=None, attention_mask=None, use_cache=None, encoder_outputs=None, **kwargs
    ):
        return {
            "input_ids": encoder_input_ids,  # encoder_outputs is defined. input_ids not needed -> save the input of encoder
            "encoder_outputs": encoder_outputs,
