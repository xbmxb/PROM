...
def squad_convert_example_to_features(
    ...
    if is_training: #and not example.is_impossible:
        ...
        if actual_text.find(cleaned_answer_text) == -1:
            ...
            # return []
        cleaned_answer_text_ = tokenizer.encode(cleaned_answer_text, add_special_tokens=False)
    if len(cleaned_answer_text_) < max_seq_length:
        for i in range(max_seq_length-len(cleaned_answer_text_)):
            cleaned_answer_text_.extend([tokenizer.pad_token_id])
...
def squad_convert_examples_to_features(
    ...
    if return_dataset == "pt":
        ...
        all_ans_text = torch.tensor([f.ans_text for f in features], dtype=torch.long)
