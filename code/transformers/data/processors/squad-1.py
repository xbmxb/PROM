...
def squad_convert_example_to_features(
    ...
    for span in spans:
        ...
        for i in range(len(sep_position)):
            if sep_position[i]!=0:
                if i==0:
                    turn_ids.extend([-2]*(sep_position[0]-cls_index+1))
                else:
                    turn_ids.extend([i-1]*(sep_position[i]-sep_position[i-1]))
...
def squad_convert_examples_to_features(
    ...
    if return_dataset == "pt":
        ...
        # print("all_is_impossible:",all_is_impossible.dtype)

        if not is_training:
            all_feature_index = torch.arange(all_input_ids.size(0), dtype=torch.long)
            dataset = TensorDataset(
                all_input_ids, all_attention_masks, all_token_type_ids, all_feature_index, all_cls_index, all_p_mask, all_sep_position, all_turn_ids, all_speaker_ids
            )
        else:
...
class SquadResult:
    ...
    def __init__(self, unique_id, start_logits, end_logits, has_log, start_top_index=None, end_top_index=None, cls_logits=None):
