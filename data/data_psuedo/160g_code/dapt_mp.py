...
def create_an_example(eg):
    ...
    for isen, sen in enumerate(sentences):
        ...
        ol = overlap(sen, left_sen, debug=args.debugging, metric=args.metric, tokenizer = tokenizer)
        ols.append((isen, ol))
    ...
    for data in used_datas:
        ...
        with open(fn, 'r', encoding='utf-8') as f:
            ...
            for idx, line_ in enumerate(tqdm(f,total=num)):
                if args.debugging and idx > 10:
                    break
                
                line = ' '.join(tokenizer.tokenize(line_.strip())) if tokenizer else line_
                sentences = line.split(". ") # simply use . as end of the sentences.
                if len(sentences) >= 2:
...
parser.add_argument('--data_dir', type=str, default='./dapt/DAPT-corpus')
parser.add_argument('--do_token', action='store_true')
parser.add_argument('--metric', type=str, default='efd')
parser.add_argument('--debugging', action ='store_true')
parser.add_argument('--datas_prefix', type=str, default=None)
...
used_datas = [ x for x in datas if x.startswith(args.datas_prefix)] if args.datas_prefix else used
